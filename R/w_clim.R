...
w_clim<- function(y_ini = c(N = 400, N = 400, N = 400),
  ...
  if(temp_cmin[1]<temp_cmax[1] && temp_cmin[2]<temp_cmax[2] && temp_cmin[3]<temp_cmax[3] ){
    ...
    wclim70 <- wclim70[[s]]
      ...
      m1<- nls(y1 ~ exp(loga + b * x), df1, start = list( loga = log(2), b = 0.005),control = list (maxiter = 500))
      y_est1<-predict(m1,df1$x)


      m2<- nls(y2 ~ exp(logw + z * x), df2, start = list(logw = log(2), z = 0.005), control = list (maxiter = 500))
      y_est2<-predict(m2,df2$x)


      m3<- nls(y3 ~ exp(loge + g * x), df3, start = list(loge = log(2), g = 0.005),control = list (maxiter = 500))
      ...
      p1<- ggplot(data, aes(x=x, y=y, group=group, fill=group)) +
               ...
               geom_ribbon(data=subset(dat2,x>times[1] & x<time_ext2),aes(x=x,ymax=y),ymin=0,alpha=0.3, fill="green4") +
               geom_ribbon(data=subset(dat3,x>times[1] & x<time_ext3),aes(x=x,ymax=y),ymin=0,alpha=0.3, fill="blue") +
               ...
               geom_line(data =subset(data1,x>times[1] & x<time_ext1), color = "brown")+
               geom_line(data =subset(data2,x>times[1] & x<time_ext2), color = "green4")+
               geom_line(data =subset(data3,x>times[1] & x<time_ext3), color = "blue")+
      ...
      p2<-ggplot(data, aes(x=x, y=y, group=group, fill=group)) +
              ...
              geom_vline(xintercept = time_ext2, size=.5, color="green4",linetype="dashed")+
              geom_vline(xintercept = time_ext3, size=.5, color="blue",linetype="dashed")+
              geom_line(data =subset(da1,x>times[1] & x<time_ext1), color = "brown")+
              geom_line(data =subset(da2,x>times[1] & x<time_ext2), color = "green4")+
              geom_line(data =subset(da3,x>times[1] & x<time_ext3), color = "blue")+
